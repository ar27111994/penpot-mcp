...
if (!home || !login) return { error: 'Board not found', boards: boards.map(b => b.name) };
...
back.addInteraction('click', {
  type: 'navigate-to',
  destination: home,
  animation: { type: 'slide', way: 'out', direction: 'left', duration: 300, easing: 'ease-in-out' }
});
```
...
const trigger = penpotUtils.findShapes(s => s.name === 'btn-delete', sourceboard)[0];
...
```javascript
const closeBtn = penpotUtils.findShapes(s => s.name === 'btn-close', modal)[0];
...
const loginBtn = penpotUtils.findShapes(s => s.name === 'btn-login', loginBoard)[0];
...
const showErrorBtn = penpotUtils.findShapes(s => s.name === 'btn-show-error', loginBoard)[0];
...
allBoards.slice(0, 5).forEach(board => {
  (board.interactions || []).forEach(i => { i.remove(); removed++; });
});
return { removedCount: removed, remaining: allBoards.length - 5 };
```
