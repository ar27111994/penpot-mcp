...
board.x = 100;  board.y = 200;   // ✅ for root-level shapes
...
const entryBoard = penpotUtils.findShape(s => s.name === '/flows/onboarding-start');
...
```javascript
function ensureTypography(name, fontFamilies, fontWeight, fontSize, lineHeight, letterSpacing, textTransform) {
  const existing = penpot.library.local.typographies.find(t => t.name === name);
  if (existing) return existing;
...
function ensureComponent(name, shapes) {
  ...
  component.name = name;  // supports path: 'mobile/note-row/default'
  return component;
}
...
component.name = 'mobile/note-row/default';
```
...
const token = set.addToken({
  ...
  value: '#1B6CA8'            // TokenValueString — can reference: '{color.base.500}'
});
...
function ensureTheme(group, name, sets) {
  const existing = penpot.library.local.tokens.themes
    .find(t => t.group === group && t.name === name);
  const theme = existing || penpot.library.local.tokens.addTheme({ group, name });
  sets.forEach(set => {
...
addToken(base, 'color', 'color.brand.primary', '#1B6CA8');
addToken(base, 'spacing', 'spacing.md', '16');
addToken(base, 'borderRadius', 'radius.md', '8');
addToken(base, 'dimension', 'effect.blur.glass', '20');
addToken(base, 'opacity', 'opacity.overlay', '0.85');
...
const interWeights = typos.filter(t => t.fontFamilies === 'Inter').map(t => t.fontWeight);
...
### Solid fill
```javascript
shape.fills = [{ fillColor: '#1B6CA8', fillOpacity: 1 }];
```
...
shape.fills = [{
  fillColorGradient: {
    type: 'linear',
    startX: 0.5, startY: 0,    // 0–1 normalized
    endX: 0.5, endY: 1,
    width: 1,
    stops: [
      { color: '#1B6CA8', opacity: 1, offset: 0 },
      { color: '#1B6CA8', opacity: 0, offset: 1 }
    ]
  }
}];
```
...
const imageData = await penpot.uploadMediaUrl('image-name', 'https://example.com/image.jpg');
...
shape.shadows = [{
  ...
  blur: 32,
  spread: 0,
  color: { color: '#003587', opacity: 0.06 },  // ← .color + .opacity, not r/g/b/a
  hidden: false
}];
```
...
function glassPanel(parent, name, x, y, w, h, surfaceColor = '#FFFFFF', surfaceOpacity = 0.85) {
  const panel = penpot.createRectangle();
  panel.name = name;
  panel.resize(w, h);
  panel.borderRadius = 20;
  panel.fills = [{ fillColor: surfaceColor, fillOpacity: surfaceOpacity }];
  panel.blurs = [{ type: 'layer-blur', value: 20, hidden: false }];
  panel.shadows = [{
    style: 'drop-shadow', offsetX: 0, offsetY: 8, blur: 32, spread: 0,
    color: { color: '#003587', opacity: 0.06 }, hidden: false
  }];
  // Optional: ghost border (only when accessibility requires)
  // panel.strokes = [{ strokeColor: '#1B6CA8', strokeOpacity: 0.15, strokeWidth: 1, strokeStyle: 'solid', strokeAlignment: 'inner' }];
...
const DS = { colors: { primary: '#1B6CA8', ... }, typography: [...] };
storage.designSystem = DS;
return { stored: true, colorCount: Object.keys(DS.colors).length };

// Call 2: retrieve in next call
const DS = storage.designSystem || { colors: { primary: '#1B6CA8' } };
// ← always provide fallback in case session was reset
const C = DS.colors;
...
function addToken(set, type, name, value) {
  return set.tokens.find(t => t.name === name) || set.addToken({ type, name, value: String(value) });
}
...
function ensurePage(name) {
  const existing = penpotUtils.getPageByName(name);
  if (existing) { penpot.openPage(existing); return penpot.currentPage; }
  const page = penpot.createPage();
  page.name = name;
  penpot.openPage(page);
  return page;
...
function clearBoards(prefix) {
  penpotUtils.findShapes(s => s.type === 'board' && s.name.startsWith(prefix), penpot.root)
    .forEach(s => s.remove());
}
```
...
const pages = penpotUtils.getPages().map(p => {
  ...
  return { name: p.name, boardCount: page.findShapes({ type: 'board' }).length };
});
...
return {
  ...
  textStyleSample: [...new Set(
    allShapes.filter(s => s.type === 'text')
...
```javascript
const pageDefs = [
  { name: 'Foundations', w: 1728, h: 1220 },
  { name: 'Mobile', w: 1440, h: 1200 },
  { name: 'Tablet', w: 1500, h: 980 },
  { name: 'Web', w: 1600, h: 1040 },
  { name: 'Widgets', w: 1500, h: 980 },
];
...
const css = penpot.generateStyle(selection[0], { type: 'css', includeChildren: true });
return css;
```
...
type Action = NavigateTo | OpenOverlay | ToggleOverlay | CloseOverlay | PreviousScreen | OpenUrl;
```
...
home.addInteraction('after-delay', { type: 'navigate-to', destination: detail }, 2000);

// Open overlay
const modal = penpotUtils.findShape(s => s.name === 'overlay/confirm-delete');
home.addInteraction('click', {
  type: 'open-overlay', destination: modal,
...
const tinyText = penpotUtils.findShapes(s => s.type === 'text' && Number(s.fontSize) < 12, penpot.root);
const hardCodedFills = penpotUtils.findShapes(s =>
  s.fills?.some(f => f.fillColor && !f.fillColorRefId), penpot.root);

// Naming checks
const autoNamed = penpotUtils.findShapes(
  s => /^(Rectangle|Ellipse|Text|Group|Frame|Board)\s*\d+$/.test(s.name), penpot.root);

// Prototype coverage
const unwiredBoards = allBoards.filter(b => !b.interactions?.length).map(b => b.name);
const brokenInteractions = allBoards.flatMap(b =>
  (b.interactions || []).map(i => ({
    source: b.name, dest: i.action.destination?.name,
...
return {
  ...
  unwiredBoardCount: unwiredBoards.length, unwiredBoards,
  brokenInteractions,
  tokenSetNames,
...
### Tablet (768×1024) — 35/65 split pane
- Rail nav: 88px wide; List pane: 35%; Detail pane: 65%
- Overlay: 50% screen width; respects system insets
...
platforms.forEach(({ name, w, h }) => {
  ...
  b.name = name; b.resize(w, h); b.x = x; b.y = 0;
  x += w + 100;
});
```
...
| `radius-overlay` | 20px | Glassmorphism overlays |
